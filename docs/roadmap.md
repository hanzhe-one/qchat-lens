...
| # | 主题 | 优先级 | 说明 |
|---|---|---|---|
| ~~A~~ | ~~真 Agent 前端 + 文案~~ | P1 | ✅ 完成：AI 重新识别按钮 + 诚实文案；实测 134 条 131 条 AI 分类 |
| A2 | **收集箱扫描改异步**（新发现）| P1 | reclassify 全量走 LLM 是同步阻塞，134 条 >120s，前端会转圈/超时。应改成后台任务 + 进度轮询（复用 analyze 的 running/done 模式）|
