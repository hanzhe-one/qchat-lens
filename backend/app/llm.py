...
{
  "items": [
    {"id": 消息id, "tags": ["该消息的标签", ...]}   // 逐条打标：为每条有实质内容的消息打 1~4 个具体标签
  ],
  "tags": ["本批汇总标签", ...],        // 3~8 个覆盖全批的多维标签，供概览用
...
规则：
- items 里的 id 必须是输入中真实出现的 id；相近主题的消息可共用标签，但不要求全批一致
- 标签要具体可检索，不要用"聊天"/"消息"这种泛词
- 纯表情、纯问候等无实质内容的消息可从 items 中省略（即不打标签）
